...
sub OP_subsumes {
    ...
    my $web = $Foswiki::Plugins::DBCachePlugin::Core::dbQueryCurrentWeb
      || $baseWeb;
...
sub handleSIMILARTOPICS {
    ...
    my @similarTopics =
      $hierarchy->getSimilarTopics( $thisTopic, $theThreshold );
    return '' unless @similarTopics;

    my %wmc = ();
    map { $wmc{$_} = $hierarchy->computeSimilarity( $thisTopic, $_ ) }
      @similarTopics;
...
sub handleCATINFO {
    ...
    foreach my $catName ( sort @$categories ) {
        ...
        if ( $theMatchCase eq 'on' ) {
            next
              if $theExclude && $category->{$theMatchAttr} =~ /^($theExclude)$/;
            next
              if $theInclude && $category->{$theMatchAttr} !~ /^($theInclude)$/;
        }
        else {
            next
              if $theExclude
                  && $category->{$theMatchAttr} =~ /^($theExclude)$/i;
            next
              if $theInclude
                  && $category->{$theMatchAttr} !~ /^($theInclude)$/i;
        }
        ...
        next
          if $subsumesCat && !$hierarchy->subsumes( $subsumesCat, $category );
        ...
        if ( $line =~ /\$parent/ ) {
            @parents =
              sort { uc( $a->{title} ) cmp uc( $b->{title} ) }
              $category->getParents($parentSubsumesCat);
        ...
        if ( $line =~ /\$children/ ) {
            ...
            if ( $theHideNull eq 'on' ) {
                @children = grep { $_->countLeafs() > 0 } @children;
            }

            if ( $theSortChildren eq 'on' ) {
                @children =
                  sort {
                         $b->countLeafs() <=> $a->countLeafs()
                      || $a->{title} cmp $b->{title}
                  } @children;
            }
        ...
        $line =~ s/\$tags/$tags/g;
        $line =~ s/,/&#44;/g
          ;   # hack around MAKETEXT where args are comma separated accidentally
...
sub handleTAGINFO {
    ...
    foreach my $tag ( sort @tags ) {
        ...
        if ( $context->{SolrPluginEnabled} ) {
            ...
            $url = Foswiki::Func::getScriptUrl( $thisWeb, "WebTagCloud", "view",
                tag => $tag );
        }
        my $class = $tag;
        $class =~ s/["' ]/_/g;
        $class = "tag_" . $class;
        my $link =
"<a href='$url' rel='tag' class='\$class'><noautolink>$tag</noautolink></a>";
        $line =~ s/\$index/$index/g;
        $line =~ s/\$url/$url/g;
        $line =~ s/\$link/$link/g;
...
sub beforeSaveHandler {
    ...
    my $doAutoReparent =
      Foswiki::Func::getPreferencesFlag( 'CLASSIFICATIONPLUGIN_AUTOREPARENT',
        $web );
    ...
    if ( Foswiki::Func::topicExists( $theFormWeb, $theForm ) ) {
        try {
            ...
            my $e = shift;
            print STDERR
"ERROR: can't read form definition $theForm in ClassificationPlugin::Core::beforeSaveHandler\n";
        };
        if ($formDef) {
            foreach my $fieldDef ( @{ $formDef->getFields() } ) {
                writeDebug(
                    "formDef field $fieldDef->{name} type=$fieldDef->{type}");
                $isCatField{ $fieldDef->{name} } = 1
                  if $fieldDef->{type} eq 'cat';
                $isTagField{ $fieldDef->{name} } = 1
                  if $fieldDef->{type} eq 'tag';
            }
        }
    }
    ...
    if (FIXFORMFIELDS) {
        ...
        foreach my $field ( $meta->find('FIELD') ) {
            ...
            if ( $isCatField{ $field->{name} } ) {
                ...
                $field->{value} =~
                  s/^top=.*$//;    # clean up top= in value definition
                my $item;
                $field->{value} = join(
                    ', ',
                    map {
    ...
    if ( $formName =~ /^Applications[\.\/]ClassificationApp[\.\/]Category$/ ) {
        ...
        /^Applications[\.\/]ClassificationApp[\.\/]CategorizedTopic$/ )
    {
        $newTopicType{CategorizedTopic} = 1;
        $newTopicType{WikiTopic}        = 0;
    }
    elsif (
        $formName =~ /^Applications[\.\/]ClassificationApp[\.\/]TaggedTopic$/ )
    {
        $newTopicType{TaggedTopic} = 1;
        $newTopicType{WikiTopic}   = 0;
    }
    elsif ( $formName =~
        /^Applications[\.\/]ClassificationApp[\.\/]ClassifiedTopic$/ )
    {
    ...
    if ( keys %newTopicType ) {
        my @newTopicType;
        foreach my $item (
            sort { $newTopicType{$b} <=> $newTopicType{$a} }
            keys %newTopicType
          )
        {
    ...
    return
      unless $topicType =~
          /ClassifiedTopic|CategorizedTopic|Category|TaggedTopic/;
    ...
    if ( $purgeMode < 2 ) {
        my $hierarchy = getHierarchy($web);
        my $catFields =
          $hierarchy->getCatFields( split( /\s*,\s*/, $topicType ) );
...
sub getCacheFile {
    ...
    return Foswiki::Func::getWorkArea("ClassificationPlugin") . '/' . $web
      . '.hierarchy';
}
...
sub getHierarchy {
    ...
    $web =~ s/\//\./go;
    if (  !$loadTimeStamps{$web}
        || $loadTimeStamps{$web} < getModificationTime($web) )
    {

        #writeDebug("constructing hierarchy for $web");
        require Foswiki::Plugins::ClassificationPlugin::Hierarchy;
        $hierarchies{$web} =
          new Foswiki::Plugins::ClassificationPlugin::Hierarchy($web);
...
sub renameTag {
    ...
    foreach my $topic (@$topics) {
        ...
        foreach my $from (@from) {
            if ( $tags{$from} ) {
                $gotAccess =
                  Foswiki::Func::checkAccessPermission( 'change', $user, undef,
                    $web, $topic )
        ...
        if ($found) {
            ...
            $meta->putKeyed( 'FIELD',
                { name => 'Tag', title => 'Tag', value => $newTags } );
...
sub getIndexFields {
    ...
    try {
        ...
        my $e = shift;
        print STDERR
"ERROR: can't read form definition $formName in ClassificationPlugin::Core::getIndexFields\n";
    };
        ...
        foreach my $fieldDef ( @{ $formDef->getFields() } ) {
            ...
            if ( $type eq 'cat' ) {
                ...
                foreach my $category ( keys %thisCategories ) {
                    push @$indexFields,
                      [ 'field_' . $name . '_flat_lst' => $category ];
                }
                ...
                my $fieldName =
                    'field_' 
                  . $name
                  . '_lst';    # Note, there's a field_..._s as well
                foreach my $category ( keys %thisCategories ) {
