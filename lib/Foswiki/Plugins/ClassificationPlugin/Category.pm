...
sub countLeafs {
    ...
    unless ( defined $nrLeafs ) {
        ...
        if ($filter) {
            my $db = Foswiki::Plugins::DBCachePlugin::Core::getDB(
                $this->{hierarchy}->{web} );
...
sub _getAllParents {
    ...
    if ( !defined( $this->{allparents} ) && !$seen->{ $this->{name} } ) {
        ...
        foreach my $parent (@parents) {
            $this->{allparents}{ $parent->{name} } = 1
              unless $parent eq $this->{hierarchy}{_top};
            $this->{allparents} = {
                %{ $this->{allparents} },
                %{ $parent->_getAllParents( $parent, $seen ) }
            };
        }
        $this->{gotUpate} = 1;
    }
...
sub getTopics {
    ...
    unless ( defined( $this->{_topics} ) ) {
        ...
        my $hierarchy = $this->{hierarchy};
        my $db =
          Foswiki::Plugins::DBCachePlugin::Core::getDB( $hierarchy->{web} );
...
sub checkAccessPermission {
    ...
    unless ( defined $access ) {
        ...
        $access =
          Foswiki::Func::checkAccessPermission( $type, $user, undef, $topic,
            $web );
...
sub getPreferences {
    ...
    unless ( $this->{_prefs} ) {
        ...
        $this->{_prefs} =
          new Foswiki::Prefs::PrefsCache( $this->{hierarchy}->{_prefs},
            undef, 'CAT', $this->{origWeb}, $this->{name} );
    }
...
sub importCategories {
    ...
    foreach my $impCat ( split( /\s*,\s*/, $impCats ) ) {
        ...
        my ( $impWeb, $impTopic ) =
          Foswiki::Func::normalizeWebTopicName( $thisWeb, $impCat );
        $impWeb =~ s/\//\./go;
   ...
   # SMELL: prevent deep recursion of two webs importing each other's categories
        my $impHierarchy =
          Foswiki::Plugins::ClassificationPlugin::getHierarchy($impWeb);
...
sub getIconUrl {
    ...
    return $pubUrlPath . '/Applications/ClassificationApp/IconSet/' . $icon;
...
sub getLink {
    ...
    return
        "<a href='"
      . $this->getUrl()
      . "' rel='tag' class='$this->{name}'><noautolink>$this->{title}</noautolink></a>";
}
...
sub getUrl {
    ...
    if ( $hierWeb ne $this->{origWeb} ) {
        return Foswiki::Func::getScriptUrl( $hierWeb, 'TopCategory', 'view',
            catname => $this->{name} );
    }

    return Foswiki::Func::getScriptUrl( $hierWeb, $this->{name}, 'view' );
}
...
###############################################################################
sub traverse {
    my ( $this, $params, $nrCalls, $index, $nrSiblings, $seen, $depth,
        $parentTitle )
      = @_;
    ...
    $format =
'<ul><li> <a href="$url"><img src="$icon" />$title</a> ($leafs) $children</li></ul>'
      unless defined $format;

    # get sub-categories
    my @children =
      sort { $a->{order} <=> $b->{order} || $a->{title} cmp $b->{title} }
      map { $this->{children}{$_} }
...
#writeDebug("children=".join(', ', map {$_->{name}} @children));
    ...
    my $isExpanded =
      0;    # set to true if this category is opened by a subsumed opener
    my $isOpener = 0;
    if ( $params->{open} ) {
        $doChildren = 0;
        my %openers = ();
        if ( defined $params->{_openers} ) {
            ...
            my $openers =
              Foswiki::Plugins::ClassificationPlugin::Core::expandVariables(
                $params->{open},
...
#print STDERR "$this->{name}: isOpener=$isOpener, isExpanded=$isExpanded, doChildren=$doChildren\n";
    ...
    if ($doChildren) {
        ...
        foreach my $child (@children) {
            next if $child->{name} eq 'BottomCategory';
            my $childResult =
              $child->traverse( $params, $nrCalls, $childIndex, $nrChildren,
                $seen, $isExpanded ? $depth : $depth + 1,
                $this->{title} );
    ...
    if ( $matchCase eq 'on' ) {
        return $subResult
          if defined $params->{exclude}
              && $this->{$matchAttr} =~ /^($params->{exclude})$/;
        return $subResult
          if defined $params->{include}
              && $this->{$matchAttr} !~ /^($params->{include})$/;
    }
    else {
        return $subResult
          if defined $params->{exclude}
              && $this->{$matchAttr} =~ /^($params->{exclude})$/i;
        return $subResult
          if defined $params->{include}
              && $this->{$matchAttr} !~ /^($params->{include})$/i;
    }
    ...
    return $subResult
      if defined $params->{hidenull}
          && $params->{hidenull} eq 'on'
          && !$nrTopics;
    ...
    return $subResult
      if defined $params->{duplicates}
          && $params->{duplicates} eq 'off'
          && $params->{seen}{ $this->{name} };
    ...
    if ($subResult) {
        $header = Foswiki::Plugins::ClassificationPlugin::Core::expandVariables(
            $header,
            'web'         => $this->{hierarchy}->{web},
            'origweb'     => $this->{origWeb} || '',
            'topic'       => $this->{name},
        ...
        $footer = Foswiki::Plugins::ClassificationPlugin::Core::expandVariables(
            $footer,
            'web'         => $this->{hierarchy}->{web},
            'origweb'     => $this->{origWeb} || '',
            'topic'       => $this->{name},
    ...
    return Foswiki::Plugins::ClassificationPlugin::Core::expandVariables(
        $format,
