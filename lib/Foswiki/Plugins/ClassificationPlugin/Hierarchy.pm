...
sub new {
    ...
    my $this;
    my $cacheFile =
      Foswiki::Plugins::ClassificationPlugin::Core::getCacheFile($web);
    ...
    unless ($refresh) {
        eval { $this = Storable::lock_retrieve($cacheFile); };
    }

    if ( $this && $this->{_version} == OBJECTVERSION ) {
        writeDebug(
            "restored hierarchy object (v$this->{_version}) from $cacheFile");
...
sub finish {
    ...
    if ($gotUpdate) {
        ...
        my $cacheFile =
          Foswiki::Plugins::ClassificationPlugin::Core::getCacheFile(
            $this->{web} );
...
sub init {
    ...
    # be anal
    die "recursive call to Hierarchy::init for $this->{web}"
      if $insideInit{ $this->{web} };
    ...
    foreach my $topicName ( $db->getKeys() ) {
        ...
        if ( $topicType =~ /\bCategory\b/ ) {
            ...
            $cat->importCategories( $form->fastget("ImportedCategory"),
                $seenImport );
        }
    }
...
sub getSimilarTopics {
    ...
    foreach my $topicB ( $db->getKeys() ) {
        ...
        my $similarity =
          $this->computeSimilarity( $topicA, $topicB, $tagsA, $catsA );
...
sub computeSimilarity {
    ...
    map {
        defined( $catsB{$_} ) ? $intersection += CATWEIGHT : $onlyA +=
          CATWEIGHT
    } @$catsA;
...
sub traverse {
    ...
    my @cats =
      ...
      map { $this->getCategory($_) } split( /\s*,\s*/, $top );

    my $nrSiblings = scalar(@cats);
    foreach my $cat (@cats) {
        if ($cat) {
            my $catResult =
              $cat->traverse( $params, \$nrCalls, 1, $nrSiblings, $seen );
    ...
    $separator =
      Foswiki::Plugins::ClassificationPlugin::Core::expandVariables($separator);
...
sub checkAccessPermission {
    ...
    unless ( defined $aclAttribute ) {
        $aclAttribute = Foswiki::Func::getPreferencesValue(
            'CLASSIFICATIONPLUGIN_ACLATTRIBUTE',
            $this->{web} )
          || 'Category';
